...
<title>Pelaporan Kokurikulum SK Morni Pok - Stabil</title>
...
<h2 class="font-semibold mt-6">Gambar Aktiviti (max 3 gambar)</h2>
...
<!-- Template PDF -->
<div id="report">
<div class="report-page p-6 text-sm bg-white">
...
const r_images = document.getElementById('r_images');
r_images.innerHTML = '';
...
// process gambar
const files = [...images.files].slice(0,3);
...
loadedImages.forEach(img => r_images.appendChild(img));
...
Dropbox.save({files:[{url: URL.createObjectURL(file), filename:file.name}], success: function(){alert('PDF berjaya disimpan ke Dropbox.');}});
}
</script>
